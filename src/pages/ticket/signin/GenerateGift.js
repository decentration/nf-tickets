  ...
  const validateGiftAmount = (amount) => {
    ...
    if (totalChainAmount && balance) {
      ...
      if (remainingBalance.lt(chainInfo?.existentialDeposit || 0)) {
        const keepAliveError = `The ticket amount of ${amount} ${chainInfo.token} will bring your account balance below ${edAmount} ${chainInfo.token} (existential deposit for the ${chainInfo?.chainName} network). This will kill your account and make you lose the remaining funds.`;
