...
export default function FAQ ({ giftTheme }) {
  ...
  const a1 = (
    <p className="pb-3">
      {giftTheme.network} is a decentralised organism of humans interacting via a decentralised technology,
      enabling humans to interact with each other and trust each in new ways.&nbsp;
  ...
  const a2 =
    `${giftTheme.network} Pizza lets you generate a unique gift secret that functions as a voucher to claim an amount of ${giftTheme.content}. Gift givers can send the unique gift secret to a friend or family member, and they will be guided through the process of claiming the gift and setting up a ${giftTheme.network} address. The gift secret should not be shared with anyone other than the intended recipient, as anyone with access to it can claim the gift.`;
  const a3 =
  ...
  const a6 = (
    <p className="pb-3">
      Please visit our&nbsp;
      <a href="https:/kabocha.network/" rel="noreferrer" target="_blank">support page</a>.
    </p>
  );
